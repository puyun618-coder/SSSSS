...
const GestureController: React.FC<GestureControllerProps> = ({ setTreeState, setHandPosition }) => {
  ...
  useEffect(() => {
    ...
    const initMediaPipe = async () => {
      ...
      try {
        ...
        try {
            // Attempt 1: Try GPU Delegate to perform faster and avoid CPU logs
            ...
            // Attempt 2: Fallback to CPU if GPU fails
            if (!isMounted) return;
  ...
  useEffect(() => {
    ...
    // Robust startup check using setInterval to catch when video is actually ready
  ...
  const handleVideoLoad = () => {
      if(videoRef.current) {
          ...
          // Attempt to start loop immediately on event, handled safely by predictWebcam checks
          if (!rafId.current) predictWebcam();
      }
  };
